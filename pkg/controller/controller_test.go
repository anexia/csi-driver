...
var _ = Describe("Controller Service", func() {
	...
	Context("CreateVolume", func() {
		...
		It("can create volumes", func() {
			...
			volume := dv.Volume{
				...
				Size:                    12345,
				ADSClass:                "ENT2",
				StorageServerInterfaces: &[]dv.StorageServerInterface{{Identifier: testStorageServerIdentifier}},
			}
			...
			// Get StorageServer
			engine.EXPECT().Get(gomock.Any(), &dv.StorageServerInterface{Identifier: testStorageServerIdentifier}).DoAndReturn(func(_ any, v *dv.StorageServerInterface, _ ...any) error {
				v.IPAddress = dv.IPAddress{
			...
			// Create
			engine.EXPECT().
				Create(gomock.Any(), &volume).DoAndReturn(func(_ any, v *dv.Volume, _ ...any) error {
			...
			// AwaitCompletion
			engine.EXPECT().Get(gomock.Any(), &createdVolume).DoAndReturn(func(_ any, v *dv.Volume, _ ...any) error {
		...
		It("returns an error when the configured storage server couldn't be retrieved", func() {
			engine.EXPECT().Get(gomock.Any(), &dv.StorageServerInterface{Identifier: testStorageServerIdentifier}).Return(api.ErrNotFound)
	...
	Context("ControllerGetCapabilities", func() {
		It("responds with the expected capabilities", func() {
			resp, err := cs.ControllerGetCapabilities(context.TODO(), &csi.ControllerGetCapabilitiesRequest{})
			Expect(err).ToNot(HaveOccurred())
			Expect(resp).ToNot(BeNil())
			Expect(resp.Capabilities).To(Equal([]*csi.ControllerServiceCapability{
				{
	...
	Context("DeleteVolume", func() {
		It("can delete volumes", func() {
			...
			engine.EXPECT().Destroy(gomock.Any(), &dv.Volume{Identifier: testVolumeIdentifier})
		...
		It("returns a Internal error when the volume couldn't be deleted", func() {
			...
			engine.EXPECT().Destroy(gomock.Any(), &dv.Volume{Identifier: testVolumeIdentifier}).Return(api.NewHTTPError(500, "DELETE", nil, nil))
	...
	Context("ValidateVolumeCapabilitiesRequest", func() {
		...
		It("returns an error when the requested volume doesn't exist", func() {
			engine.EXPECT().Get(gomock.Any(), &dv.Volume{Identifier: "foo"}).Return(api.NewHTTPError(404, "GET", nil, nil))
			resp, err := cs.ValidateVolumeCapabilities(context.TODO(), &csi.ValidateVolumeCapabilitiesRequest{
				VolumeId:           "foo",
				VolumeCapabilities: []*csi.VolumeCapability{{}},
			})
		...
		It("returns an error when the requested volume capabilities are not supported", func() {
			engine.EXPECT().Get(gomock.Any(), &dv.Volume{Identifier: "foo"}).Return(nil)
			resp, err := cs.ValidateVolumeCapabilities(context.TODO(), &csi.ValidateVolumeCapabilitiesRequest{
				VolumeId:           "foo",
